  ...
  res.on('end', () => {
    try {
      ...
      const opportunities = Object.values(audits)
        .filter(audit => audit.details && audit.details.type === 'opportunity' && (audit.details.overallSavingsMs > 0 || audit.details.overallSavingsBytes > 0))
        .sort((a, b) => (b.details.overallSavingsMs || 0) - (a.details.overallSavingsMs || 0));
        
      opportunities.slice(0, 5).forEach(op => {
        const savings = op.details.overallSavingsMs ? `Saves ~${Math.round(op.details.overallSavingsMs)}ms` : `Saves ~${Math.round(op.details.overallSavingsBytes / 1024)}KB`;
        console.log(`- [${op.title}]: ${savings} (${op.description.replace(/\[Learn more\].*/g, '')})`);
      });
      
      console.log('\n--- DIAGNOSTICS ALERTS ---');
      const diagnostics = ['render-blocking-resources', 'unused-javascript', 'modern-image-formats', 'uses-optimized-images', 'dom-size', 'font-display'];
